...
war_dat %>% 
  group_by(playerId) %>%
  summarize(start_year = min(yearId), end_year = max(yearId), years_played = n(), total_war = round(sum(rWAR), 2)) %>%
  head(10) %>% dust %>% sprinkle_print_method("html")
```
